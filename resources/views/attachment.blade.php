...
<body>
    <div class="container-fluid">
        ...
        <form style="width: 100%;" method="post" action="{{ url('AttachmentSave') }}" enctype="multipart/form-data">
            ...
            <input type="hidden" name="InvoiceNo" value="{{session::get('VHNO')}}" id="InvoiceNo">
            <input type="file" name="filenames[]" multiple="" class="form-control form-control-sm"
                accept="application/pdf, application/vnd.ms-excel" style="width: 75%;">
        ...
        $attachment = DB::table('attachment')
            ->where('InvoiceNo', session::get('VHNO'))
            ->get();
                ...
                <tbody>
                    @foreach ($attachment as $key => $value)
                        <tr>
                            ...
                            <td class="col-md-1 text-center"> <a href="{{ URL('/documents/' . $value->FileName) }}"
                                    target="_blank"><i class="mdi mdi-eye-outline  align-middle me-1"></i></a> <a
                                    href="{{ URL('/AttachmentDelete/' . $value->AttachmentID . '/' . $value->FileName) }}"
                                    onclick="return confirm('Are you sure you want to delete this item?');"><i
                                        class="bx bx-trash  align-middle me-1"></i></a> </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
